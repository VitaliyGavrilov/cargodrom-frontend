...
@Directive()
export abstract class TableConstructor <T extends { id: number }, A = never, F = never> implements OnInit, OnDestroy {
  ...
  protected abstract getTableConfig (par:any): Observable<any>;
  protected abstract getTableRows <T>(params?: LoadParams<T, F>): Observable<{ total: number, items: T[], column?: string[], sort?: string[],sort_new?:any }>;
  ...
  protected loadRows(): void {
    ...
    const params = { start: this.start, count: this.count, sort: JSON.stringify(sortCol) as unknown as SortColumn<T>[], ...this.filter  };
    this.getTableRows(params)
      .subscribe(rows => {
